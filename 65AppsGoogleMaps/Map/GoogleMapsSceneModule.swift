...
class GoogleMapsSceneModule {
    ...
    init(networkManager: NetworkDataProvider, fakeNetworkManager: FakeNetworkDataProvider, dataModel: [POIItem], clusterConfigurator: ClusterManager) {
        
    self.view = MapViewController.make()
        
        guard self.view != nil else { return }
        let router = GoogleMapsRouter(view: self.view)
        let interactor = GoogleMapsInteractor(networkManager: networkManager, fakeNetworkManager: fakeNetworkManager)
